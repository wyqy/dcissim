...
rng(142857, 'simdTwister'); seed = uint32(142857);
...
while ~is_stable
    ...
    is_stable = max(abs(eig(temp_sys.A, 'vector'))) <= 0.95;
end
...
%% 使用EM算法求解
% 参数选取
em_yn_size = 2e3;  % 由于速度较慢, 使用较少的采样点
turns = 100;  % 执行轮数
% 调用函数
[em_cov_x, em_cov_y, em_xn, em_residual] = emEstimation(yn(:, 1:em_yn_size), x0, mat_a, mat_c, 'end_condition', 'iteration', 'turn', turns);
...
disp(['Eigenvalue of A: ', mat2str(abs(eig(temp_sys.A, 'vector')), 2)]);
...
% 绘图
% 状态估计相对误差
% figure; semilogy(abs((em_xn(:, 2:end)-xn(:, 1:em_yn_size)./abs(xn(:, 1:em_yn_size)))).'); legend;
% 迭代误差
...
%% 针对LTI模型的EM算法
function [est_cov_x, est_cov_y, est_xn, est_residual] = emEstimation(yn, x0, mat_a, mat_c, varargin)
% 假定已知初值!
% 隐参数为各个状态量
% 返回值同时还包括隐参数, 即(最后一轮的)状态变量的估计值
    ...
    addParameter(parser, 'turn', 1e3, @(i)(isnumeric(i)&&isscalar(i)));
    ...
    % 参数准备
    turn = 0;
    end_turn = min(1e3, end_turn);  % 最多迭代次数
    ...
    while true
        ...
        % 最优隐参数估计
        % Kalman初始化, t = 0~N
        kalman_xn_now_now(:, 1) = x0;
        kalman_cov_xn_now_now(:, :, 1) = zeros(x_size, x_size);
        for iter_k = 2:sample_size+1  % t = 1~N
            ...
            kalman_gain_now = kalman_cov_xn_now_past(:, :, iter_k) * mat_c.' * pinv(mat_c * kalman_cov_xn_now_past(:, :, iter_k) * mat_c.' + est_cov_y);
        ...
        % 反向迭代
        for iter_k = sample_size:-1:1  % t = 1~N-1
            % 辅助矩阵
            conexp_j = kalman_cov_xn_now_now(:, :, iter_k) * mat_a * pinv(kalman_cov_xn_now_past(:, :, iter_k+1));
            if iter_k >= 2, conexp_j_past = kalman_cov_xn_now_now(:, :, iter_k-1) * mat_a * pinv(kalman_cov_xn_now_past(:, :, iter_k)); end
            % 计算
            conexp_xn(:, iter_k) = kalman_xn_now_now(:, iter_k) + conexp_j * (conexp_xn(:, iter_k+1) - mat_a * kalman_xn_now_now(:, iter_k));
            conexp_cov_xn_now_now(:, :, iter_k) = kalman_cov_xn_now_now(:, :, iter_k) + conexp_j * (conexp_cov_xn_now_now(:, :, iter_k+1) - kalman_cov_xn_now_past(:, :, iter_k+1)) * conexp_j.';
            if iter_k >= 2, conexp_cov_xn_now_past(:, :, iter_k) = kalman_cov_xn_now_now(:, :, iter_k) * conexp_j_past.' + conexp_j * (conexp_cov_xn_now_past(:, :, iter_k+1) - mat_a * kalman_cov_xn_now_now(:, :, iter_k)) * conexp_j_past.'; end
        end
        % 条件期望计算
        conexp_para_cov_x_mat1 = sum(conexp_cov_xn_now_now(:, :, 2:sample_size), 3) + conexp_xn(:, 2:sample_size) * conexp_xn(:, 2:sample_size).';
        conexp_para_cov_x_mat2 = sum(conexp_cov_xn_now_past(:, :, 3:sample_size+1), 3) + conexp_xn(:, 3:sample_size+1) * conexp_xn(:, 2:sample_size).';
        conexp_para_cov_x_mat3 = sum(conexp_cov_xn_now_now(:, :, 3:sample_size+1), 3) + conexp_xn(:, 3:sample_size+1) * conexp_xn(:, 3:sample_size+1).';
        ...
        conexp_para_cov_y = mat_c * sum(conexp_cov_xn_now_now(:, :, 3:sample_size+1), 3) * mat_c.' + conexp_para_cov_y_diff(:, 3:sample_size+1) * conexp_para_cov_y_diff(:, 3:sample_size+1).';

        %%% maximization: 对显参数的最大化
        argmax_cov_x = conexp_para_cov_x./(sample_size-1);
        argmax_cov_y = conexp_para_cov_y./(sample_size-1);

        %%% 迭代条件检查
        turn = turn + 1;
        % 计算均方根误差, 并存储
        residual_1 = sum((est_cov_x - argmax_cov_x).^2,'all');
        residual_2 = sum((est_cov_y - argmax_cov_y).^2,'all');
        est_residual(turn) = sqrt((residual_1 + residual_2)/(x_size*x_size + y_size*y_size));
        % 判断终止条件 (注意: 最后一次的迭代值会被舍弃)
        % conditional break point: est_residual(turn) > 1
        switch end_type
            case 'residual'
                if (est_residual(turn) < end_residual) || isnan(est_residual(turn)) || (turn >= end_turn)
                    disp(['第', num2str(turn-1), '次EM算法参数估计完成']);
        ...
        % 强行跳出条件
        if min(eig(argmax_cov_x)) < 0 || min(eig(argmax_cov_y)) < 0
            disp(['第', num2str(turn-1), '次EM算法参数估计完成(提前跳出)']);
            disp(['参数均方根误差为: ', num2str(est_residual(turn))]);
            est_xn = conexp_xn;
            break;
        end
...
%% 简化ALS - ICM估计法
function est_cov_xy = alsEstimation(moment_yy, mat_a, mat_c)
% 输出-状态协方差 - 不存在互协方差
% 此处计算真实矩
%#ok<*EQEFF>
%#ok<*VUNUS>
    ...
    for iter_moment = 1:order-1
        est_para_m(location_base+1:location_base+y_size, :) = mat_c*mpower(mat_a, iter_moment);
        location_base = location_base + y_size;
    end
    % n
    est_para_n = kron(mat_c, est_para_m);
    % b
    est_para_b = zeros((order-1)*y_size, y_size);
    location_base = 0;
    for iter_moment = 1:order-1
        est_para_b(location_base+1:location_base+y_size, :) = moment_yy(:, :, iter_moment+1);
        location_base = location_base + y_size;
    end
    est_para_b = reshape(est_para_b, [(order-1)*y_size*y_size 1]);
    % PC^T估计
    est_mat_pct = lsqminnorm(est_para_m, est_para_b);
    est_mat_pct = reshape(est_mat_pct, [x_size y_size]);
    % P估计
    cvx_begin sdp quiet;
        variable est_mat_p(x_size, x_size) symmetric;
        minimize(norm(est_para_n * vec(est_mat_p) - est_para_b)); %, Inf));
        subject to;
            est_mat_p*(mat_c.') == est_mat_pct;
            est_mat_p == semidefinite(x_size);
    cvx_end;

    % 估计cov_y : R
    est_cov_y = moment_yy(:, :, 1) - mat_c * est_mat_p * mat_c.';
    % 估计cov_x : Q
    est_cov_x = est_mat_p - mat_a * est_mat_p * mat_a.';

    % 返回值
    est_cov_xy = blkdiag(est_cov_x, est_cov_y);
