function out = genRandom(generator_info, period_sample, signal_size, ~)
%GENRANDOM random 信号生成
% 给定信号表达式, 返回[0, N-1]下的截断信号
% 为减少不同周期间的冲激, 逐元素减去y = (sig(N-1)-sig(0))/(N-1) * n的值
% 输出size @ (size period_sample)

    % 参数提取
    random_mu = generator_info.random_mu;
    random_covariance = generator_info.random_covariance;
    % 参数修正
    ...
    for iter_signal = 1:signal_size
        out(iter_signal, :) = randn(noise_seed{iter_signal}, 1, period_sample);
        % 修正起终点
