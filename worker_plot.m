...
exportgraphics(fig_bode, '..\..\%latex%\derivation - mod4\fig\siso_bode_1.eps')

%% SISO covariance plot
analysis_cov_location = 48;
fig_cov = anaPlotCov(analysis_outcov_series(:, :, analysis_cov_location));
exportgraphics(fig_cov, '..\..\%latex%\derivation - mod4\fig\siso_covariance_1.eps')

%% SISO covariance histogram
% fig_cov = anaPlotCov(analysis_outcov_series(:, :, analysis_cov_location));
fig_hist = anaPlotHistogram(analysis_outcov_error_norm);
exportgraphics(fig_hist, '..\..\%latex%\derivation - mod4\fig\siso_covariance_hist_1.eps')

%% MIMO recursive plot
fig_recursive = anaPlotRecursive(analysis_h2, 10);
exportgraphics(fig_recursive, '..\..\%latex%\derivation - mod4\fig\mimo_recursive_1.eps')

%% MIMO scatter plot
fig_scatter = anaPlotScatter(analysis_spot_inout_3d);
set(fig_scatter,'renderer','Painters');
exportgraphics(fig_scatter, '..\..\%latex%\derivation - mod4\fig\mimo_covariance_1.eps')
