...
% 输入信号 - 随机部分
para_sim_x_snr = 5;  % 状态信号基础信噪比
para_sim_y_snr = 5;  % 输出信号基础信噪比
para_sim_u_snr = 5;  % 输入信号基础信噪比
...
%% 验证辨识结果
analysis_original_x_size = para_xsize;
...
% 方差验证
analysis_covariance_out_size = 10;
analysis_original_covariance = sim_result_struct.covariance;
[analysis_original_moments, analysis_original_hnorms] = anaCov(analysis_original_covariance(1:analysis_original_x_size+analysis_y_size, 1:analysis_original_x_size+analysis_y_size), sim_plant_struct.A, sim_plant_struct.C, analysis_covariance_out_size);
[analysis_identified_moments, analysis_identified_hnorms] = anaCov(iden_offline_result_struct.cov_all(1:analysis_identified_x_size+analysis_y_size, 1:analysis_identified_x_size+analysis_y_size), iden_offline_result_struct.A, iden_offline_result_struct.C, analysis_covariance_out_size);
disp(['Relatively MSE of H2 norm for two covariances are: ', mat2str(mean((analysis_original_hnorms-analysis_identified_hnorms).^2)/mean((analysis_original_hnorms).^2), 4)]);
