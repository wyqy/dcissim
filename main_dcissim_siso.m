...
% para_xsize = 4; para_ysize = 1; para_usize = 1;
% 是否全量辨识
if ~exist('para_isfull'), para_isfull = true; end
if para_isfull, para_iden_isim_excitation_type = 'full';
else, para_iden_isim_excitation_type = 'reduced'; end
...
for iter_exp = 1:para_experiment_count
    ...
    analysis_xsize(2, iter_exp) = analysis_xsize(2, iter_exp);
...
analysis_incov_error_norm = zeros(1, para_experiment_count); analysis_outcov_error_norm = zeros(2, para_experiment_count);
...
if para_isfull
    for iter_exp = 1:para_experiment_count
        ...
        temp_ind_cov = result_dcissim_cell{iter_exp}.cov(analysis_xsize(2, iter_exp)+para_ysize+1:end, analysis_xsize(2, iter_exp)+para_ysize+1:end);
...
for iter_exp = 1:para_experiment_count
    ...
    temp_ind_cov = result_dcissim_cell{iter_exp}.cov(1:analysis_xsize(2, iter_exp)+para_ysize, 1:analysis_xsize(2, iter_exp)+para_ysize);
    temp_sim_cov = anaInnoCov(result_sim_cell{iter_exp}.K, result_sim_cell{iter_exp}.NoiseVariance);
    [temp_ori_cov_rr, temp_ori_cov_zr, ~, analysis_outcov_series(1, :, iter_exp)] = anaCov(temp_ori_cov, result_original_cell{iter_exp}, analysis_cov_count);
    [temp_dcissim_cov_rr, temp_dcissim_cov_zr, ~, analysis_outcov_series(2, :, iter_exp)] = anaCov(temp_ind_cov, result_dcissim_cell{iter_exp}, analysis_cov_count);
    [temp_sim_cov_rr, temp_sim_cov_zr, ~, analysis_outcov_series(3, :, iter_exp)] = anaCov(temp_sim_cov, result_sim_cell{iter_exp}, analysis_cov_count);
    % 用于数据计算
    if para_ysize >= para_xsize
        temp_ori_cov_ww = result_original_cell{iter_exp}.cov(1:para_xsize, 1:para_xsize);
        temp_ori_cov_vv = result_original_cell{iter_exp}.cov(para_xsize+1:para_xsize+para_ysize, para_xsize+1:para_xsize+para_ysize);
        temp_ind_cov_ww = result_dcissim_cell{iter_exp}.cov(1:analysis_xsize(2, iter_exp), 1:analysis_xsize(2, iter_exp));
        temp_ind_cov_vv = result_dcissim_cell{iter_exp}.cov(analysis_xsize(2, iter_exp)+1:analysis_xsize(2, iter_exp)+para_ysize, analysis_xsize(2, iter_exp)+1:analysis_xsize(2, iter_exp)+para_ysize);
        [temp_ind_cov_ww, ~] = sys_compare(temp_ori_cov_ww, temp_ind_cov_ww); [temp_ind_cov_vv, ~] = sys_compare(temp_ori_cov_vv, temp_ind_cov_vv); analysis_outcov_error_norm(1, iter_exp) = temp_ind_cov_ww + temp_ind_cov_vv;
    else
        [temp_dcissim_cov_rr, ~] = sys_compare(temp_ori_cov_rr, temp_dcissim_cov_rr); [temp_dcissim_cov_zr, ~] = sys_compare(temp_ori_cov_zr, temp_dcissim_cov_zr); analysis_outcov_error_norm(1, iter_exp) = temp_dcissim_cov_rr + temp_dcissim_cov_zr;
        [temp_sim_cov_rr, ~] = sys_compare(temp_ori_cov_rr, temp_sim_cov_rr); [temp_sim_cov_zr, ~] = sys_compare(temp_ori_cov_zr, temp_sim_cov_zr); analysis_outcov_error_norm(2, iter_exp) = temp_sim_cov_rr + temp_sim_cov_zr;
    end
end
...
disp(['R2 of dCov(output) as norm: ', mat2str(analysis_criterion_outcov(1), 2), ' / ', mat2str(analysis_criterion_outcov(2), 2)]);
...
analysis_bode_cell = {result_original_cell{analysis_bode_location}, result_dcissim_cell{analysis_bode_location}, result_sim_cell{analysis_bode_location}};
...
save_postfix = '_mimo';
if para_isfull && save_valid
    save(['results\mat_full_auto' save_postfix '.mat'], '-v7.3');
else
    if save_valid
        save(['results\mat_reduce_auto' save_postfix '.mat'], '-v7.3');
    end
end
