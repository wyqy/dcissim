...
%% 确定参数
% 仿真系统
para_xsize = 1; para_ysize = para_xsize; para_usize = 1;
...
% 随机模型个数
if ~exist('para_experiment_count'), para_experiment_count = 5; end
...
% 初始化算法
iden_full_init = idenDCISSIMLaucher(sim_result_struct.un, 'y_size', para_ysize, 'u_size', para_usize, 'period_samples', sim_period_samples, 'cutted_periods', para_iden_cutted_period, ...
...
iden_reduce_init = idenDCISSIMLaucher(sim_result_struct.un, 'y_size', para_ysize, 'u_size', para_usize, 'period_samples', sim_period_samples, 'cutted_periods', para_iden_cutted_period, ...
...
for iter_exp = 1:para_experiment_count  %(默认para_ysize >= para_xsize)
    ...
    temp_full_cov_tt = result_full_cell{iter_exp}.cov(analysis_xsize(2, iter_exp)+para_ysize+1:end, analysis_xsize(2, iter_exp)+para_ysize+1:end);
    analysis_inoutcov_error(1, para_xsize*para_xsize+para_ysize*para_ysize+1:end, iter_exp) = reshape(temp_full_cov_tt-temp_ori_cov_tt, [], 1);
    [analysis_incov_error_norm(1, 1, iter_exp), ~] = sys_compare(temp_ori_cov_tt, temp_full_cov_tt);
    % 输出数据计算
    ...
    temp_full_cov_ww = result_full_cell{iter_exp}.cov(1:analysis_xsize(2, iter_exp), 1:analysis_xsize(2, iter_exp)); temp_full_cov_vv = result_full_cell{iter_exp}.cov(analysis_xsize(2, iter_exp)+1:analysis_xsize(2, iter_exp)+para_ysize, analysis_xsize(2, iter_exp)+1:analysis_xsize(2, iter_exp)+para_ysize);
    temp_reduce_cov_ww = result_reduce_cell{iter_exp}.cov(1:analysis_xsize(3, iter_exp), 1:analysis_xsize(3, iter_exp)); temp_reduce_cov_vv = result_reduce_cell{iter_exp}.cov(analysis_xsize(3, iter_exp)+1:analysis_xsize(3, iter_exp)+para_ysize, analysis_xsize(3, iter_exp)+1:analysis_xsize(3, iter_exp)+para_ysize);
    analysis_inoutcov_error(1, 1:para_xsize*para_xsize, iter_exp) = reshape(temp_full_cov_ww-temp_ori_cov_ww, [], 1);
    analysis_inoutcov_error(2, 1:para_xsize*para_xsize, iter_exp) = reshape(temp_reduce_cov_ww-temp_ori_cov_ww, [], 1);
    analysis_inoutcov_error(1, para_xsize*para_xsize+1:para_xsize*para_xsize+para_ysize*para_ysize, iter_exp) = reshape(temp_full_cov_vv-temp_ori_cov_vv, [], 1);
    analysis_inoutcov_error(2, para_xsize*para_xsize+1:para_xsize*para_xsize+para_ysize*para_ysize, iter_exp) = reshape(temp_reduce_cov_vv-temp_ori_cov_vv, [], 1);
    
    [~, ~, ~, analysis_outcov_series(1, :, iter_exp)] = anaCov(blkdiag(temp_ori_cov_ww, temp_ori_cov_vv), result_original, analysis_cov_count);
    [~, ~, ~, analysis_outcov_series(2, :, iter_exp)] = anaCov(blkdiag(temp_full_cov_ww, temp_full_cov_vv), result_full_cell{iter_exp}, analysis_cov_count);
    [~, ~, ~, analysis_outcov_series(3, :, iter_exp)] = anaCov(blkdiag(temp_reduce_cov_ww, temp_reduce_cov_vv), result_reduce_cell{iter_exp}, analysis_cov_count);
    
    [temp_full_cov_ww, ~] = sys_compare(temp_ori_cov_ww, temp_full_cov_ww); [temp_full_cov_vv, ~] = sys_compare(temp_ori_cov_vv, temp_full_cov_vv);
    [temp_reduce_cov_ww, ~] = sys_compare(temp_ori_cov_ww, temp_reduce_cov_ww); [temp_reduce_cov_vv, ~] = sys_compare(temp_ori_cov_vv, temp_reduce_cov_vv);
    analysis_outcov_error_norm(1, 1, iter_exp) = temp_full_cov_ww ; analysis_outcov_error_norm(1, 2, iter_exp) = temp_full_cov_ww ;
    analysis_outcov_error_norm(2, 1, iter_exp) = temp_reduce_cov_ww; analysis_outcov_error_norm(2, 2, iter_exp) = temp_reduce_cov_ww;
...
analysis_bode_cell = {result_original, result_full_cell{analysis_bode_location}, result_reduce_cell{analysis_bode_location}};
