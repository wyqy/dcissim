...
%   此处显示详细说明
    ...
    switch algo_type
        case 'offline'  % 离线辨识
            ...
            yk = yk(:, T+1:end);
            uk = uk(:, T+1:end);
            signal_sample = T*fix(size(yk, 2)/T);
            yk = yk(:, 1:signal_sample);
            uk = uk(:, 1:signal_sample);
            % ISIM辨识
            [yv_est, uv_est] = idenISIM(yk, uk, mat_v, T, 'ordinary');
            ...
            if ~strcmp(als_est_type, 'none')
                ...
                noise_para_est = idenCovariance(yk, uk, mat_v, para_est, als_est_type, aorder_prior, aorder_est_type);  % (可能)询问阶数
            end
        case 'online'  % 在线辨识
            % 记录迭代步
            k = k + 1;
