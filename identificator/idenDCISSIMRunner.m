...
%   此处显示详细说明
    ...
    persistent covariance_struct cov_all cov_kalman
    ...
    if isempty(is_inited)
        ...
        cov_all = zeros(1+y_size+u_size); cov_kalman = zeros(1, y_size); covariance_struct = struct('cov_all', cov_all, 'cov_kalman', cov_kalman);
        is_inited = 1;
    end
    ...
    switch dcissim_type
        case 'offline'  % 离线辨识
            ...
            % 方差辨识
            covariance_struct = idenCovariance(yn, un, vn, y_isim, u_isim, mat_a_sim, mat_c_sim, cutted_periods*period_samples, cov_cross_type, online_cov_order, online_cov_order_type);  % (可能)询问阶数
        case 'online'  % 在线辨识
            % 记录迭代步
            n = n + 1;
            % ISIM辨识
        ...
        otherwise, x_size_sim = 0; mat_a_sim = 0; mat_c_sim = 0; mat_b_sim = 0; mat_d_sim = 0; mat_x_sim = 0; covariance_struct = struct('cov_all', 0, 'cov_kalman', 0);
    end
