...
%     if mat_s(1) == 1  % 包含直流信号
%         % 分块位置
%         block_size = (v_size+1)/2;
%         block_start = ones(1, block_size);
%         block_end = ones(1, block_size);
%         block_length = ones(1, block_size);
%         for iter_select = 1:block_size-1
%             block_start(iter_select+1) = 2*iter_select;
%             block_end(iter_select+1) = 2*iter_select+1;
%             block_length(iter_select+1) = 2;
%         end
%         % 分块频率
%         block_angle = zeros(1, block_size);
%         for iter_select = 2:block_size
%             block_cos = mat_s(block_start(iter_select), block_start(iter_select));
%             block_sin = mat_s(block_start(iter_select), block_start(iter_select)+1);
%             block_angle(iter_select) = atan2(block_sin, block_cos);
%         end
%     else  % 不包含直流信号
%         % 分块位置
%         block_size = v_size/2;
...
%         block_end = ones(1, block_size);
%         block_length = ones(1, block_size);
%         for iter_select = 1:block_size
%             block_start(iter_select) = 2*iter_select-1;
%             block_end(iter_select) = 2*iter_select;
%             block_length(iter_select+1) = 2;
%         end
%         % 分块频率
%         block_angle = zeros(1, block_size);
...
% 
%     % 准备矩阵分解
%     decompose_character_mat_a = cell(block_size, 1);
%     for iter_ds = 1:block_size, decompose_character_mat_a{iter_ds} = decomposition(mat_a_sim - cos(block_angle(iter_ds)).*eye(x_size_sim)); end    
%     % 计算参数矩阵
%     location_base = 0;
%     if sim_ss_d_type == 0  % 假定 D = 0
%         mat_pinv = zeros(v_size*y_size, u_size*x_size_sim);
%         for iter_block = 1:block_size
%             mat_pinv(location_base+1:location_base+block_length(iter_block)*y_size, :) = -kron(u_isim(:, block_start(iter_block):block_end(iter_block)).', mat_c_sim / decompose_character_mat_a{iter_block});
%             location_base = location_base + block_length(iter_block)*y_size;
%         end
%     else  % 不做假定
%         mat_pinv = zeros(v_size*y_size, u_size*(x_size_sim+y_size));
%         for iter_block = 1:block_size
%             mat_pinv(location_base+1:location_base+block_length(iter_block)*y_size, 1:u_size*x_size_sim) = -kron(u_isim(:, block_start(iter_block):block_end(iter_block)).', mat_c_sim / decompose_character_mat_a{iter_block});
%             mat_pinv(location_base+1:location_base+block_length(iter_block)*y_size, u_size*x_size_sim+1:end) = kron(u_isim(:, block_start(iter_block):block_end(iter_block)).', eye(y_size));
%             location_base = location_base + block_length(iter_block)*y_size;
%         end
%     end
% 
%     % 计算最小二乘
%     retVec = pinv(mat_pinv) * reshape(y_isim, [v_size*y_size 1]);
% 
%     % 还原结果为矩阵
%     if sim_ss_d_type == 0  % 假定 D = 0
%         mat_b_sim = reshape(retVec, [x_size_sim u_size]);
...
% 
%     % 计算状态参数
%     mat_x_sim = zeros(x_size_sim, v_size);
%     for iter_block = 1:block_size
%         mat_x_sim(:, block_start(iter_block):block_end(iter_block)) = -decompose_character_mat_a{iter_block} \ mat_b_sim * u_isim(:, block_start(iter_block):block_end(iter_block));
%     end
% 
% end