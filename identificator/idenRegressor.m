...
% 对v的初值无要求, 因此可以随机/指定初始化
    ...
    % 返回参数计算
    excitation_frequencies = zeros(v_size, 1);
    excitation_phi = zeros(v_size, 1);
    harmonic_phi = harmonicPhaseInit(harmonic_size);
    % 直流部分(如有)
    if frequencies(1) == 0
        excitation_frequencies(1) = 0;
        excitation_phi(1) = pi/2;
        location_base = 1;
    else
        location_base = 0;
    end
    ...
    for iter = 1:harmonic_size
        excitation_frequencies(location_base+1:location_base+2) = omega*harmonic_frequencies(iter)*ones(2, 1);
        excitation_phi(location_base+1:location_base+2) = harmonic_phi(iter)*ones(2, 1);
        excitation_phi(location_base+2) = excitation_phi(location_base+2) + pi/2;
    ...
    switch regressor_type
        case 'ordinary'  % 返回计算结果
            n = 0:signal_sample-1;
            vn = sin((excitation_frequencies*n) + excitation_phi);
            regressor_struct = struct('vn', vn);
        case 'recursive'  % 返回构造矩阵
            regressor_struct = struct('excitation_frequencies', excitation_frequencies, 'excitation_phi', excitation_phi);
